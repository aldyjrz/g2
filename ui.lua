...
local Custom = {}
do
	Custom.ColorRGB = Color3.fromRGB(24, 150, 74)
	Custom.ObsidianTop = Color3.fromRGB(31, 31, 31)
	Custom.ObsidianMid = Color3.fromRGB(31, 18, 12)
	Custom.ObsidianLow = Color3.fromRGB(26, 29, 38)
	Custom.Surface = Color3.fromRGB(18, 19, 27)
	Custom.Surface2 = Color3.fromRGB(23, 22, 32)
	Custom.Surface3 = Color3.fromRGB(31, 27, 42)
	Custom.Stroke = Color3.fromRGB(47, 125, 79)
	Custom.Text = Color3.fromRGB(255, 236, 245)
	Custom.Muted = Color3.fromRGB(156, 154, 170)
	Custom.Font = Enum.Font.Ubuntu
	Custom.FontBody = Enum.Font.Ubuntu
...
local SnutzHub_Library = {}

SnutzHub_Library.Icons = Icons
SnutzHub_Library.DefaultTabIcons = DefaultTabIcons
SnutzHub_Library.Unloaded = false

function SnutzHub_Library:SetNotification(Config)
...
function SnutzHub_Library:CreateWindow(Config)
	local Title = Config[1] or Config.Title or ""
	local Description = Config[2] or Config.Description or ""
	...
	local SnutzHubGui = Custom:Create("ScreenGui", {
		ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
	}, GetGuiParent())
	...
	DiscordButton.Activated:Connect(function()
		...
		SnutzHub_Library:SetNotification({
			Title = "SnutzHub",
			Description = "Discord",
	...
	OkBtn.Activated:Connect(function()
		...
		if not SnutzHub_Library.Unloaded then
			SnutzHub_Library.Unloaded = true
		end
	end)
	...
	local PreferredTab = Config.DefaultTab or Config.DefaultTabName or SnutzHub_Library.LastSelectedTab or "Info"
	...
	local function SwitchTab(TabName)
		...
		SnutzHub_Library.LastSelectedTab = CurrentTab
		...
		function Sections:AddSection(Title, OpenSection)
			...
			function Item:AddParagraph(Config)
				...
				local function UpdateParagraphSize()
					local ContentHeight = EstimateWrappedHeight(ParagraphContent.Text, 58, 13, 1, 4)

					ParagraphContent.Size = UDim2.new(1, -20, 0, ContentHeight)
					Paragraph.Size = UDim2.new(1, 0, 0, math.max(35, ContentHeight + 32))
			...
			function Item:AddToggle(Config)
				...
				local function UpdateToggleSize()
					RowHeight = CompactRowHeight(Content)
					Toggle.Size = UDim2.new(1, 0, 0, RowHeight)
					ToggleTitle.Position = Content == "" and UDim2.new(0, 10, 0.5, -7) or UDim2.new(0, 10, 0, 7)
					ToggleContent.Visible = Content ~= ""
			...
			function Item:AddSlider(Config)
				...
				local function UpdateSliderSize()
					...
					SliderTitle.Position = Content == "" and UDim2.new(0, 10, 0.5, -7) or UDim2.new(0, 10, 0, 7)
					SliderContent.Visible = Content ~= ""
			...
			function Item:AddInput(Config)
				...
				local function UpdateInputSize()
					...
					InputTitle.Position = Content == "" and UDim2.new(0, 10, 0.5, -7) or UDim2.new(0, 10, 0, 7)
					InputContent.Visible = Content ~= ""
			...
			function Item:AddDropdown(Config)
				...
				local function UpdateBaseHeight()
					BaseHeight = CompactRowHeight(Content)
					DropdownTitle.Position = Content == "" and UDim2.new(0, 10, 0.5, -7) or UDim2.new(0, 10, 0, 7)
					DropdownContent.Visible = Content ~= ""
